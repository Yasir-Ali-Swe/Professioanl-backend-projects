...
const SubscriptionDetail = () => {
    ...
    return (
        <div className="space-y-4 sm:space-y-6 pb-8">
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-sm sm:text-base">Manual Override</CardTitle>
                    <CardDescription className="text-xs sm:text-sm">
                        Change the organization's subscription plan manually
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="flex flex-col sm:flex-row gap-4 sm:items-end">
                                ...
                                <SelectContent>
                                    {availablePlans.map((plan) => (
                                        <SelectItem key={plan._id} value={plan._id}>
                                            {capitalize(plan.name)} - ${plan.price}/{plan.billingCycle}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        ...
                        <AlertDialog>
                            <AlertDialogTrigger render={() => {
                                <Button
                                    disabled={!selectedPlan || selectedPlan === currentPlan?._id || isUpdating}
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        This will change the organization's plan from{' '}
                                        <strong>{capitalize(currentPlan?.name)}</strong> to{' '}
                                        <strong>{capitalize(availablePlans.find(p => p._id === selectedPlan)?.name)}</strong>.
                                        {selectedPlan === currentPlan?._id ? '' : (
                                            <span className="block mt-2 text-muted-foreground">
                                                {availablePlans.find(p => p._id === selectedPlan)?.price > (currentPlan?.price || 0)
                                                    ? 'The organization will be charged the new price on the next billing cycle.'
                                                    : 'The organization will be downgraded immediately.'}
                                            </span>
                                        )}
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
